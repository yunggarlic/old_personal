...
import { Object3D, MathUtils, InstancedMesh, DynamicDrawUsage, Vector3 } from 'three';
...
function useTrimesh(fn, fwdRef) {
  return useBody('Trimesh', fn, args => {
    return [args[0].map(v => v instanceof Vector3 ? [v.x, v.y, v.z] : v), args[1]];
  }, fwdRef);
}
function useConvexPolyhedron(fn, fwdRef) {
  return useBody('ConvexPolyhedron', fn, args => {
    return [args[0].map(v => v instanceof Vector3 ? [v.x, v.y, v.z] : v), args[1], args[2] && args[2].map(n => n instanceof Vector3 ? [n.x, n.y, n.z] : n)];
  }, fwdRef);
}
